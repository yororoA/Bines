...
# 确保可导入项目根 config（uid / token）
_ROOT = Path(__file__).resolve().parents[2]
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))
try:
    from config import MOMENTS_UID, MOMENTS_TOKEN
except ImportError:
    MOMENTS_UID = os.environ.get("MOMENTS_UID", "")
    MOMENTS_TOKEN = os.environ.get("MOMENTS_TOKEN", "")

MOMENTS_API_BASE_URL: str = os.environ.get("MOMENTS_API_BASE_URL", "http://127.0.0.1:9999")
MOMENTS_API_TIMEOUT: int = int(os.environ.get("MOMENTS_API_TIMEOUT", "15"))

# 动态工具调用日志：与 moments.py 同目录
...
def _headers() -> dict:
    """鉴权：Authorization Bearer + uid，从配置读取。"""
...
def get_moments() -> dict:
    """
    获取已发布动态列表（isEditing 固定为 false，与前端 getMoments 一致）。uid/token 从配置读取。
...
def add_moment(title: str, content: str = "") -> dict:
    """
    发布动态。published 固定 true，acknowledge 固定 false，无文件/描述，uid/token 从配置读取。
...
def get_liked_comments() -> dict:
    """
    获取当前用户点赞过的评论 id 列表。uid/token 从配置读取。
...
def get_liked_moments() -> dict:
    """
    获取当前用户点赞过的动态 id 列表。uid/token 从配置读取。
    返回: { "message": "ok", "data": [id1, id2, ...] }
    """
    _check_requests()
    r = requests.get(
